<template>
  <div>
    ...
    <Panel v-if="!fetching && order">
      ...
      <div class="row">
        <div class="col-12">
          <h3>Pagamento: <span>{{ $t(`paymentMethod.${order.paymentMethod}.label`) }}</span></h3>

          <h3>Situação: <span class="badge" v-bind:class="order.status">{{ $t(`order.statuses.${order.status}.label`) }}</span></h3>
